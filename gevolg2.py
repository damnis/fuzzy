...
def kleinste_speler_gevolg():
    knop_key = f"kleinste_knop_{st.session_state.get('vraag_index', 0)}"
    if st.button("👶 Bekijk resultaat", key=knop_key):
        if not st.session_state.get(knop_key):
            st.session_state[knop_key] = True
            speel_geluid("laugh")
